...
| `DATA_MODEL.md` | Per modificare DB, schema Drizzle, Zod, endpoint payload. | ~3k |
| `FISCAL_ENGINE.md` | **Per qualsiasi modifica a calcoli fiscali, INPS, tasse.** Contiene scenari test A-I. | ~3k |
| `API.md` | Per aggiungere/modificare endpoint REST. | ~1.5k |
...
| Nuovo endpoint REST | MASTERGUIDE + LLM_RULES + API + DATA_MODEL |
...
2. **FISCAL_ENGINE è intoccabile senza scenari test.** Qualsiasi modifica deve passare A-I.
...
- **v5.1 (corrente)** — App standalone, foglio diventa export, modello dati esteso (clienti, PAC con costi, secchielli con target, CRUD totale), tipo_inps biforcato, INPS fisso sempre.
